...
# Scopes the mentors@berkeley.edu credentials are authorized for. The refresh
# token generated by generate_token.py must be granted this exact set, so keep
# these in sync with that script.
...
class GoogleGmailSender:
    ...
    ## Build credentials entirely from environment-backed settings.
    ## No file I/O and no interactive browser flow, so this is safe to run
    ## on a headless production dyno.
    def _get_credentials(self):
        ...
        if not (client_id and client_secret and refresh_token):
            raise ImproperlyConfigured(
                "Gmail notifications require CSM_GOOGLE_GMAIL_CLIENT_ID, "
