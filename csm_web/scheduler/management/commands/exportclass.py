...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        students = Profile.objects.filter(course=course, active=True, role=Profile.STUDENT)
        # write columns
        cols = ("Day", "Start time", "Room", "End time", "Mentor email", "Student email")
        self._write(cols)
        for stud in students:
            ...
            row = (
                ...
                st.location,
                (self._get_time(st.start_time) + st.duration).time().strftime("%I:%M %p"),
