...
from scheduler.models import Attendance, Spacetime, Student, day_to_number, week_bounds
...
so long as they aren't unexecused absences"""
    ...
    def handle(self, *args, **options):
        ...
        for student in Student.objects.all().prefetch_related(
            ...
            Prefetch(
                "attendance_set", queryset=Attendance.objects.all().order_by("date")
            ),
        ):
            days = student.section.spacetimes.values_list("day_of_week", flat=True)
            for _, attendances in groupby(
                student.attendance_set.all(),
                lambda attendance: week_bounds(attendance.date),
            ):
                attendances = list(attendances)
                for attendance, day in zip(attendances, days):
                    if attendance.date.strftime("%A") != day:
                        if not any(
                            attendance.date.strftime("%A") == day
                            for attendance in attendances
                        ):
                            attendance.date = attendance.date + timedelta(
                                days=day_to_number(day) - attendance.date.weekday()
                            )
                            attendances_to_update.append(attendance)
                        elif (
                            attendance.presence != Attendance.Presence.UNEXCUSED_ABSENCE
                        ):
                            attendance.delete()
