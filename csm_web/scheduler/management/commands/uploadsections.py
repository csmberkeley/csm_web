...
from scheduler.models import Course, DayOfWeekField, Mentor, Section, Spacetime, User
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "course_name", type=str, help="The name of the course to make sections for"
        )
        parser.add_argument(
            "csv_path", type=str, help="The filepath or URL of the CSV file to be read"
        )
        parser.add_argument(
            "--validateonly",
            action="store_true",
    ...
    def validate(self, reader):
        ...
        for row in reader:
            ...
            assert (
                len(row["description"])
                <= Section._meta.get_field("description").max_length
            )
    ...
    def create_sections(self, course, reader):
        ...
        for row in reader:
            ...
            user, _ = User.objects.get_or_create(
                email=row["mentor_email"], username=row["mentor_email"].split("@")[0]
            )
            mentor = Mentor.objects.create(user=user, course=course)
            spacetimes = [
                Spacetime.objects.create(
                    location="Online",
                    day_of_week=row["day1"],
                    start_time=self.parse_time(row["time1"]),
                    duration=self.SECTION_DURATIONS[course.name],
                    section=None,
                ),
            ]
            if row["day2"] and row["time2"]:
                spacetimes.append(
                    Spacetime.objects.create(
                        location="Online",
                        day_of_week=row["day2"],
                        start_time=self.parse_time(row["time2"]),
                        duration=self.SECTION_DURATIONS[course.name],
                        section=None,
                    )
                )
